...
CxPolicy[result] {
	...
	result := {
		...
		"issueType": "MissingAttribute",
		"keyExpectedValue": sprintf("'RestrictPublicBuckets' should be defined and set to true in the 'PublicAccessBlockConfiguration'", [name]),
		"keyActualValue": sprintf("'RestrictPublicBuckets' is not defined in the 'PublicAccessBlockConfiguration'", [name]),
...
CxPolicy[result] {
	...
	result := {
		...
		"keyExpectedValue": sprintf("'RestrictPublicBuckets' should be set to true", [name]),
		"keyActualValue": sprintf("'RestrictPublicBuckets' is set to false", [name]),
