...
def print_performance_report(analysis: Dict):
    ...
    print(f"   API Calls Reduced: {analysis['old_api_calls'] - analysis['new_api_calls']} ({((analysis['old_api_calls'] - analysis['new_api_calls']) / analysis['old_api_calls'] * 100):.1f}%)")
...
def run_actual_test(queries: List[Dict]):
    ...
    try:
        ...
        from src.agent.explainer import explain_findings
